...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://postgres:postgres@localhost:5432/customer_churn_db"
)
...
def init_db_and_tables():
    sys_url = "postgresql://postgres:postgres@127.0.0.1:5432/postgres"
    conn = psycopg2.connect(sys_url)
    conn.autocommit = True

    with conn.cursor() as cur:
        cur.execute("SELECT 1 FROM pg_database WHERE datname = 'customer_churn_db'")
        if not cur.fetchone():
            cur.execute("CREATE DATABASE customer_churn_db;")
    conn.close()

    target_url = "postgresql://postgres:postgres@127.0.0.1:5432/customer_churn_db"
